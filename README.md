...
>jobs의 실행 창
![img](https://user-images.githubusercontent.com/85490910/240144614-1f2ec8fe-9c2c-4c3f-adb1-5f9dd58e3af7.PNG)

>맨 앞의 숫자는 실행된 순서를 나타낸다
>
>'&'가 없으면 멈춰있는 상태 '&'가 있으면 실행중인 상태이다

## kill

### kill의 사용
- 동작 하고 있는 프로세스에 간단한 메세지(signal)을 보내는 명령어
- 주로 프로세스를 종료하는 용도로 사용

### kill의 옵션
- -9 : 프로세스아이디(PID)를 직접 지정하여 종료
- -ㅣ : signal로 사용할수 있는 signa 이름들을 보여줌

### kill 신호의 의미
> SIGHUP : 프로그램이 사용하는 설정 파일을 변경시키고 변화된 내용 적용에 사용

>SIGINT : 현재 작동죽인 프로그램의 동작을 멈출때 사용

>SIGKILL : 프로그램을 무조꾼 종료

>SIGSEGV : 잘못된 메모리 관리시 생기는 신호

>SIGTERM : 실행중인 프로그램을 정상적인 종료 방법으로 종료

>SIGCOUNT : 중지되어있는 프로그램 재실행

>SIGTOP : 프로그램 중지

> SIGTSTP : 터미널에서 중지되어있는 
